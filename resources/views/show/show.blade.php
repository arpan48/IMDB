...
@section('content')
    ...
    <div class="row">
        <div class="col-md-3">
            <img class="card-img-top cardimg" src="{{url('img/uploaded/'.$show->poster)}}"
                 alt="{{$show->title}} poster">
            <p class="text">
                Categories:
                @foreach($link as $links)
                    @foreach($category as $categories)
                        @if($show->id == $links->movie_id && $links->category_id == $categories->id)
                            {{$categories->name}}
                        @endif
                    @endforeach
                @endforeach
        ...
        <div class="col-md-9">
            ...
            <div class="embed-responsive embed-responsive-16by9">
                <iframe class="embed-responsive-item" src="{{url('https://www.youtube.com/embed/'.$show->trailer)}}"
                        frameborder="0"
